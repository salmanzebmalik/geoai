...
class YOLO26Pipeline:
    ...
    sliding-window tiling for large-scale rasters, utilizing native
    Ultralytics scaling/letterboxing and correct window-to-geospatial coordinate mapping.
    """

    def __init__(
            self,
            model_path: str = "app/models/download_models/yolo26/best_yolo26l_v1.pt",
            conf_min: float = 0.15,  # Aligned with verified detection threshold
            imgsz: int = 1024,
            tile_size: int = 1024,
            overlap: int = 205,  # 20% overlap (~819px stride)
    ):
    ...
    def predict_boxes_geojson(self, image_bytes: bytes) -> dict:
        ...
        with rasterio.MemoryFile(image_bytes) as memfile, memfile.open() as src:
            ...
            # Determine sliding windows uniformly for both small and large rasters
            if width <= self.tile_size and height <= self.tile_size:
                windows = [rasterio.windows.Window(0, 0, width, height)]
            else:
            ...
            for window in windows:
                ...
                # Pass native window array directly; Ultralytics handles scaling and letterboxing internally.
                ...
                for box in results[0].boxes:
                    ...
                    # Map local tile pixel coordinates directly to master geospatial CRS
